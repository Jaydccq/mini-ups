...
export const RealTimeFleetMap: React.FC<FleetMapProps> = ({ className = '' }) => {
  ...
  // Calculate map bounds for 100x100 grid with trucks
  const mapData = useMemo(() => {
    // Fixed 100x100 grid bounds
    const bounds = {
      minX: 0,
      maxX: 100,
      minY: 0,
  ...
  const fetchFleetLocations = async () => {
    try {
      ...
      if (data.data && data.data.trucks) {
        ...
        // If no trucks, show mock data for demonstration
        const mockTrucks: TruckLocation[] = [
          ...
          },
          {
            ...
            status: "IN_TRANSIT",
            location: { x: 85, y: 45 },
            driver: { id: 3, name: "Mike Wilson" },
            currentShipment: {
              trackingNumber: "UPS789012",
              destination: { x: 95, y: 85 }
            },
            lastUpdate: new Date().toISOString(),
          }
        ];
      ...
      <CardContent className="space-y-4">
        ...
        <div className="relative">
          <div className="w-full h-[400px] flex items-center justify-center bg-gray-50 rounded-lg border">
            ...
            >
              {/* 100x100 Grid lines for reference */}
              <defs>
                <pattern id="grid" width="5" height="4" patternUnits="userSpaceOnUse">
                  <path d="M 5 0 L 0 0 0 4" fill="none" stroke="#e5e7eb" strokeWidth="0.3"/>
                </pattern>
                <pattern id="majorGrid" width="50" height="40" patternUnits="userSpaceOnUse">
                  <path d="M 50 0 L 0 0 0 40" fill="none" stroke="#d1d5db" strokeWidth="1"/>
                </pattern>
              </defs>
              ...
              <g className="text-xs fill-gray-500">
                {[0, 25, 50, 75, 100].map(x => (
                  <text key={`x-${x}`} x={mapData.transformX(x)} y={mapData.svgHeight - 5} textAnchor="middle">
                    {x}
                  </text>
                ))}
                {[0, 25, 50, 75, 100].map(y => (
                  <text key={`y-${y}`} x={5} y={mapData.transformY(y)} textAnchor="start" dominantBaseline="middle">
                    {y}
                  </text>
                ))}
              </g>
