...
export class NlqMcpServer {
  ...
  constructor() {
    ...
    logger.updateConfig({
      ...
      format: this.config.logging.format as any,
  ...
  private async handleHealthCheck(
    ...
    if (!includeDetails) {
      ...
      delete (result.components.orchestrator as any).details;
      delete (result.components.openrouter as any).details;
      delete (result.components.backend as any).details;
    }
  ...
  private async handleClearCache(
    ...
    try {
      ...
      requestLogger.error('Cache clearing failed', { component, error: (error as Error).message }, error as Error);
  ...
  async shutdown(): Promise<void> {
    ...
    try {
      ...
      logger.error('Error during shutdown', { error: (error as Error).message }, error as Error);
    }
  }
}
