...
export class BackendConnector {
   ...
   */
  async executeStep<T = any>(step: BackendStep): Promise<BackendResponse<T>> {
    ...
    for (let attempt = 0; attempt <= this.config.retry.maxRetries; attempt++) {
      try {
        const response = await this.makeHttpRequest<T>(step);
        
        // Success - record for circuit breaker
        this.recordSuccess();
        
        const result: BackendResponse<T> = {
          success: true,
          data: response.data,
        ...
        if (!this.isRetryableError(error as any) || attempt === this.config.retry.maxRetries) {
          break;
        }
   ...
   */
  async executeStepsParallel<T = any>(
    steps: BackendStep[],
    ...
    const results: BackendResponse<T>[] = [];
    ...
    for (let i = 0; i < steps.length; i += maxConcurrency) {
      ...
      const batchPromises = batch.map(step => this.executeStep<T>(step));
   ...
   */
  private async makeHttpRequest<T>(step: BackendStep): Promise<AxiosResponse<T>> {
   ...
   */
  private buildUrl(step: BackendStep): string {
   ...
   */
  private resolveBaseUrl(step: BackendStep): string {
    const override = step.baseUrlOverride?.trim();
    if (!override) return this.config.baseUrl;

    if (/^https?:\/\//i.test(override)) {
      return override;
    }

    if (override === 'backend') return this.config.baseUrl;
    if (override === 'amazon') {
   ...
   */
  private generateCacheKey(step: BackendStep): string {
    const keyData = {
      method: step.method,
      endpoint: step.endpoint,
   ...
   */
  private getCachedResponse<T>(key: string): BackendResponse<T> | null {
   ...
   * Cache successful response
   */
  private setCachedResponse<T>(key: string, response: BackendResponse<T>): void {
