...
export interface AppConfig {
  ...
  openrouter: {
    ...
    modelStrict: string;
    modelStrictFallback: string;
    modelAnswer: string;
    modelAnswerFallback: string;
  };
  ...
  cache: {
    enabled: boolean;
    ttlIntentParsing: number;
    ttlBackendResponse: number;
    ttlFinalAnswer: number;
    maxSize: number;
  };
  ...
  query: {
    ...
    fallbackToGenericThreshold: number;
  };
  ...
  development: {
    ...
    mockBackendEnabled: boolean;
    evaluationMode: boolean;
    debugIntentParsing: boolean;
    debugBackendCalls: boolean;
    debugAnswerGeneration: boolean;
  };
}
...
export function getModelConfig(strategy: 'fast' | 'strict' | 'answer') {
  const config = getConfig();
