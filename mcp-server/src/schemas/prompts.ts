...
import { IntentType } from './intent-plan.js';
...
export const MODEL_INSTRUCTIONS: Record<string, ModelInstruction> = {
  FAST_MODEL: {
    temperature: 0.1,
    maxTokens: 1000,
...
export const INTENT_PARSING_TEMPLATE: PromptTemplate = {
...
export const ANSWER_GENERATION_TEMPLATE: PromptTemplate = {
...
export function getModelInstructions(modelType: 'fast' | 'strict' | 'answer'): ModelInstruction {
  switch (modelType) {
    case 'fast':
...
export function createFallbackPrompt(query: string, reason: string): string {
  return `用户查询: "${query}"
