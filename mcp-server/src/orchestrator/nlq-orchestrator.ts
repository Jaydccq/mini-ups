...
export interface NlqResult {
  ...
  warnings: string[];
  debugInfo?: {
    intentParsingResult?: IntentParsingResult;
    executionResult?: ExecutionResult;
    rawLlmResponses?: Array<{ stage: string; response: string }>;
  };
}
...
export interface OrchestratorConfig {
  ...
  validateIntentConfidence: boolean;
...
export class NlqOrchestrator {
  ...
  private queryCache = new Map<string, NlqResult>();
  ...
  private async parseIntent(
    ...
    try {
      ...
      try {
        ...
        throw new Error(`Invalid JSON response from intent parsing: ${(parseError as Error).message}`);
      }
      ...
      logger.error('Intent parsing failed', { error: (error as Error).message });
   ...
   */
  private async executeBackendSteps(plan: IntentPlan): Promise<ExecutionResult> {
    const startTime = Date.now();
    ...
    const backendResponses = await backendConnector.executeStepsParallel(
      plan.steps, 
      maxConcurrency
    );
  ...
  private async generateAnswer(
    ...
    try {
      ...
      logger.error('Answer generation failed', { error: (error as Error).message });
      ...
      return {
        ...
        warnings: [`Answer generation failed: ${(error as Error).message}`]
      };
    }
  }
  ...
  private async generateFallbackAnswer(
    ...
    try {
      ...
      logger.error('Fallback answer generation failed', { error: (error as Error).message });
   ...
   */
  private enhancePromptWithContext(basePrompt: string, context?: QueryContext): string {
   ...
   */
  private generateMockBackendResponse(step: any, plan: IntentPlan): any {
   ...
   */
  private generateQueryId(query: string, context?: QueryContext): string {
   ...
   */
  private estimateTotalCost(tokens: number): number {
